...
SELECT app_name , rating , category FROM analytics 
    WHERE (rating , category) IN (SELECT MAX(rating) , category FROM analytics
        WHERE min_installs >= 50000 GROUP BY category)
    ORDER BY category;
...
SELECT app_name FROM analytics WHERE genres = (SELECT COUNT(genres) FROM analytics GROUP BY genres HAVING COUNT(genres) > 1); 
...
SELECT * FROM analytics 
WHERE genres::text LIKE '%Education%';
